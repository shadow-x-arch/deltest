...
export const FlightsPage: React.FC = () => {
  ...
  const filteredProducts = useMemo(() => {
    let filtered = products.filter(product => 
      product.category === 'Travel' && product.subcategory === 'Flights'
    );
    ...
    if (priceRange !== 'all') {
      if (priceRange === 'low') {
        ...
        filtered = filtered.filter(product => product.amount >= 1000 && product.amount < 1500);
      } else if (priceRange === 'high') {
        filtered = filtered.filter(product => product.amount >= 1500);
      }
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3">
              ...
              <motion.div
                ...
                className="mb-8"
              >
                <div className={`p-8 rounded-xl border bg-gradient-to-b from-slate-200 via-blue-50 to-white dark:from-slate-800 dark:via-blue-900/30 dark:to-gray-900 border-slate-200 dark:border-slate-700 relative overflow-hidden`}>
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className={`text-lg text-slate-700 dark:text-slate-300 opacity-90`}>
                          Direct routes to global destinations with premium service
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      {['Business Class Available', 'Direct Routes', 'Premium Service', 'Miles Rewards'].map((feature, index) => (
                        <div key={index} className={`flex items-center gap-2 text-slate-700 dark:text-slate-300 opacity-80`}>
                          <div className={`w-2 h-2 rounded-full bg-slate-700 dark:bg-slate-300 opacity-60`} />
                          <span className="text-sm font-medium">{feature}</span>
                        </div>
                      ))}
                    </div>
                    
                    <div className={`text-sm text-slate-700 dark:text-slate-300 opacity-70`}>
                      {filteredProducts.length} flights available • Earn miles with every booking
                    </div>
                  </div>
              ...
              >
                <SearchBar
                  ...
                  placeholder="Search flights by destination, airline, or route..."
                />
              </motion.div>
